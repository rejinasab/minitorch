...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert_close(1 - sig_a, sigmoid(-a))
    assert_close(sigmoid(0.0), 0.5)
    # Non-strict: for very large |a| both sides saturate to the same
    # float64 value (e.g. sigmoid(90) == sigmoid(91) == 1.0), so we allow
    # equality and only require it never *decreases*.
...
def test_symmetric() -> None:
    ...
    """
    assert_close(mul(2.5, -3.7), mul(-3.7, 2.5))
    assert_close(mul(0.0, 4.2), mul(4.2, 0.0))
...
def test_other() -> None:
    ...
    """
    # Double negation returns the original value: -(-x) = x
    for x in (0.0, 3.14, -7.5):
